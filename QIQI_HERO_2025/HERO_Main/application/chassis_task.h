...
#define CHASSIS_FOLLOW_GIMBAL_PID_KD 2000.0f
...
typedef enum
{
    ...
    CHASSIS_VECTOR_TWIST,              // 底盘摇摆模式
    CHASSIS_VECTOR_TURN_ROUND,         // 一键掉头模式
} chassis_mode_e;
...
{
    fp32 tx_vz_set;
	uint8_t tx_send_flag_b;
    uint8_t reserve; // 保留位
    uint8_t tx_chassis_mode;
    uint8_t tx_send_Flag;
} comm_tx_B_t;
typedef struct __attribute__((__packed__))
{
    fp32 tx_PITCH;
    int16_t tx_first_speed;
	int16_t tx_add_speed;
} comm_tx_C_t;
// 接收
typedef struct __attribute__((__packed__))
{
    uint16_t rx_current_heat;
    uint8_t rx_robo_level;
    uint16_t rx_initial_speed_x100;
    uint8_t rx_flag;
} comm_rx_a_t;
...
typedef struct
{
  ...
  fp32 chassis_back_init_angle;//一键掉头初始角度
  
  bool_t twist_init_flag;//摇摆初始化
  int16_t change_twist_flag;

  power_ctrl_t power_control;//功率控制实例
  ...
  // 获取裁判系统数据
  ext_robot_state_t *robot_state; // 裁判系统机器人状态
  ext_power_heat_data_t *power_heat_data;
  ext_shoot_data_t *shoot_data;

  // 双板通信数据
  // 发送
  comm_tx_A_t comm_tx_A; // 双板通信数据
  uint8_t comm_A_output[8]; // can发送的数据帧
  comm_tx_B_t comm_tx_B; // 双板通信数据
  uint8_t comm_B_output[8]; // can发送的数据帧
  comm_tx_C_t comm_tx_C;    // 双板通信数据
  uint8_t comm_C_output[8]; // can发送的数据帧
  // 接收
  comm_rx_a_t comm_rx_a;
 ...
 */
extern void
chassis_task(void const *pvParameters);
