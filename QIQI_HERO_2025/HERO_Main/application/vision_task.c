...
void vision_task(void const* pvParameters)
{
    ...
    while(1){
        ...
        vision_set_target_armor_color(&vision_control, get_robot_id());
        // 判断是否识别到目标
...
//    update->solve_trajectory.current_bullet_speed = update->bullet_speed.est_bullet_speed;
	update->solve_trajectory.current_bullet_speed = 15.6f;
    //获取目标数据
...
static void vision_data_process(vision_control_t* vision_data)
{
    ...
    if (vision_data->vision_target_appear_state == TARGET_APPEAR && vision_data->target_data.p < EKF_CONVERGENCE_P){
        ...
        if (AUTO_GIMBAL_YAW_MODE == 1 && fabs(vision_data->target_data.v_yaw) > 1.0f ||
            AUTO_GIMBAL_YAW_MODE == 0 && fabs(vision_data->target_data.v_yaw) > 10.0f){
...
void receive_decode(uint8_t* buf, uint32_t len)
{
    ...
    if (verify_CRC16_check_sum(buf, len)){
        ...
        if (temp_packet.header == HIGH_TO_LOWER_HEAD){
			...
			if (vision_receive.receive_packet.id == ARMOR_OUTPOST &&vision_receive.receive_packet.v_yaw > 0.3f)
			{
				vision_receive.receive_packet.v_yaw = 2.513f;
			}
			vision_receive.receive_packet.r1 = 0.305f;
//      	vision_receive.receive_packet.z = 0.86;
...
{   
    ...
    solve_trajectory->predict_time = solve_trajectory->flight_time + solve_trajectory->time_bias + vision_data->latency_time;
    //中心坐标位置预测
