...
float pitchTrajectoryCompensation(float s, float z, float v, float *t, float k);

float completeAirResistanceModel(float s, float v, float angle);

float monoDirectionalAirResistanceModel(float s, float v, float angle ,float *t, float k);
...
fp32 allow_attack_error = 0.01f;
fp32 time_bias = 0.0f;
extern int8_t VISION;
...
static void vision_task_init(vision_control_t* init)
{
    ...
    // 获取机器人状态指针
    init->robot_state_point = get_robot_status_point();
    // 获取发射机构弹速指针
    ...
    //获取当前位置指针
    init->auto_move.game_robot_pos = get_robot_pos_point();
...
//        fp32 pitch_set = calc_target_position_pitch_angle(&vision_data->solve_trajectory, sqrt(pow(vision_data->robot_gimbal_aim_vector.x, 2) + pow(vision_data->robot_gimbal_aim_vector.y, 2)) - vision_data->solve_trajectory.distance_static, vision_data->robot_gimbal_aim_vector.z + vision_data->solve_trajectory.z_static);
        fp32 pitch_set = -pitchTrajectoryCompensation(sqrtf((vision_data->robot_gimbal_aim_vector.x) * (vision_data->robot_gimbal_aim_vector.x) + (vision_data->robot_gimbal_aim_vector.y) * (vision_data->robot_gimbal_aim_vector.y)) - DISTANCE_STATIC, vision_data->robot_gimbal_aim_vector.z + Z_STATIC, vision_data->solve_trajectory.current_bullet_speed, &vision_data->solve_trajectory.flight_time, vision_data->solve_trajectory.k1);
...
static void calc_current_bullet_speed(vision_control_t* calc_cur_bullet_speed, bullet_type_e bullet_type, shooter_id_e shooter_id)
{
    ...
    {
        if (calc_cur_bullet_speed->shoot_data_point->shooter_number == shooter_id)
        {
            //筛选不合理的数据
            if (calc_cur_bullet_speed->shoot_data_point->initial_speed >= MIN_SET_BULLET_SPEED && calc_cur_bullet_speed->shoot_data_point->initial_speed <= MAX_SET_BULLET_SPEED)
            {
                //修正弹速 -- 修正精度到整数
                calc_cur_bullet_speed->bullet_speed = calc_cur_bullet_speed->shoot_data_point->initial_speed;
            }
        }
    }
}
...
float monoDirectionalAirResistanceModel(float s, float v, float angle ,float *t, float k)
{
    ...
    *t = (float)((expf(k * s) - 1) / (k * v * arm_cos_f32(angle)));
    if(t < 0)
    {
...
float pitchTrajectoryCompensation(float s, float z, float v, float *t, float k)
{
    ...
    for (i = 0; i < 20; i++)
    {
        ...
        z_actual = monoDirectionalAirResistanceModel(s, v, angle_pitch, t, k);
        if(z_actual == 0)
        {
            angle_pitch = 0;
            break;
