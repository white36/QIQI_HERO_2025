...
typedef struct
{
  ...
  // 接收
  comm_rx_A_t comm_rx_A; // 双板通信数据帧
  comm_rx_B_t comm_rx_B; // 双板通信数据帧
  comm_rx_C_t comm_rx_C; // 双板通信数据帧
  uint8_t rx_chassis_mode; // 储存接收到的底盘模式
  fp32 rx_PITCH_angle; // 储存接收到的pitch角度

  TickType_t CHASSIS_xTickCount; // 获取freertos系统时钟滴答数
} chassis_move_t;
