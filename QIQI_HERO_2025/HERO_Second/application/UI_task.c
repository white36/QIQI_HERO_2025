...
void UI_task(void const *pvParameters)
{
    ...
    while (1)
    {
		...
		if (last_KEY_z && !KEY_z)
		{
			if (ui_control.UI_xTickCount - ui_control.KEY_z_press_time >= 500) // 长按
			{
				// 刷新UI
				UI_draw(&ui_control);
			}
...
void UI_draw(UI_control_t *ui_control)
{
    ...
    Line_Draw(&ui_control->aim_data[3], "AL4", UI_Graph_ADD, 5, UI_Color_Yellow, 2, 920, 385, 1000, 385); // 最下面那条
